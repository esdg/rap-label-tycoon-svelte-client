<script lang="ts">
	...
	$: rappers = discoveredArtists.filter(isRapper);
	$: beatmakers = discoveredArtists.filter(isBeatmaker);

	// Loading/error helpers
	$: activeLoading = $labelDiscoveredArtistsQuery.isLoading && labelDiscoveredIds.length > 0;
	$: activeError = $tasksQuery.error || $contractsQuery.error || $labelDiscoveredArtistsQuery.error;
...
<PageContentWithScroll>
	<div class="mb-10 flex select-none flex-wrap items-start justify-between gap-4">
		<div>
			<h1 class="text-7xl font-thin uppercase leading-none">Discovered Artists</h1>
			<p class="text-sm text-gray-300">All artists you have discovered so far.</p>
		</div>
	...
	{:else if discoveredArtists.length === 0}
